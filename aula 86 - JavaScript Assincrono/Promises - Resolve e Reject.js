...
function esperaAi(msg, tempo){
    ...
    //Reject ==> Deu um erro no codigo rejeita isso aqui para mim. Na hora que uma promessa for rejeitada ele cai no catch()
    return new Promise((resolve, reject) => {// estamos chamando o constructor da Promisse e dentro dele vamos receber uma função.
        if (typeof msg !== 'string'){
            reject('BAD VALUE');
        }
        setTimeout(() => {
            resolve(msg);//vc esta resolvendo sua promisse logo depois que o setTimeout foi executado. OU SEJA ELE ESPEROU O TIMEOUT TERMINAR PARA RESOLVER A PROMISSE, PORQUE ELE SABIA QUE ERA ALI QUE IRIA DEMORAR
...
esperaAi('Conexão com o BD', rand(1, 3))
    ...
    .then(resposta => { // esse parametro do then é o parametro que esta dentro do resolve
        console.log(resposta)
        return esperaAi(22222, rand(1, 3)) // esta retornando uma outra promisse
    })
    .then(resposta => {
        console.log(resposta);
    })
    ...
    .catch(erro => {
        console.log('Erro: ', erro)// o parametro erro é o que esta dentro do reject da promisse crianda na função esperaAi 
    })// o reject cai aqui, o catch é executado quando o reject é chamado
